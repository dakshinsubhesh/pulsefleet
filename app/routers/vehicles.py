...
async def list_vehicles(
    ...
    min_capacity_kg: Optional[float] = Query(default=None, ge=0),
    max_capacity_kg: Optional[float] = Query(default=None, ge=0),
    ...
    effective_capacity_min = (
        min_capacity_kg if min_capacity_kg is not None else capacity_min
    )
    effective_capacity_max = (
        max_capacity_kg if max_capacity_kg is not None else capacity_max
    )

    if (
        effective_capacity_min is not None
        and effective_capacity_max is not None
        and effective_capacity_min > effective_capacity_max
    ):
        return JSONResponse(
            status_code=422,
            content={
                "detail": "min_capacity_kg cannot be greater than max_capacity_kg.",
                "error_code": "invalid_capacity_range",
            },
        )
    ...
    if capacity_min is not None:
        filters.append(models.Vehicle.capacity_kg >= effective_capacity_min)
    if capacity_max is not None:
        filters.append(models.Vehicle.capacity_kg <= effective_capacity_max)
    if effective_search is not None:
        pattern = f"%{effective_search}%"
