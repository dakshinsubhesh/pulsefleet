...
async def update_vehicle(
    ...
    """Partially update a vehicle. Only fields present in the request body are changed."""
    result = await db.execute(select(models.Vehicle).where(models.Vehicle.id == vehicle_id))
    vehicle = result.scalar_one_or_none()
    if vehicle is None:
        raise NotFoundError(f"Vehicle {vehicle_id} not found.", error_code="vehicle_not_found")
...
async def delete_vehicle(
    ...
    Explicit rule: a vehicle with any non-terminal shipment (pending or
    in_transit) cannot be deleted, for the same reason as drivers — it
    would sever a real, active assignment.
    """
    result = await db.execute(select(models.Vehicle).where(models.Vehicle.id == vehicle_id))
    vehicle = result.scalar_one_or_none()
    if vehicle is None:
