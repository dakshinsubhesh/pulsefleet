...
async def list_drivers(
    q: Optional[str] = Query(
        ...
        description="Case-insensitive substring match on name, license_number, or phone",
    ),
    status_filter: Optional[models.DriverStatus] = Query(default=None, alias="status"),
    ...
    `q` searches name, license_number, and phone (case-insensitive
    substring, matched with SQL ILIKE — no in-memory filtering). `status`
    filters exactly. Both combine with the owner scope via AND in SQL.
    """
    filters = [models.Driver.owner_id == current_user.id]
    if status_filter is not None:
        filters.append(models.Driver.status == status_filter)
    if q is not None:
        pattern = f"%{q}%"
        filters.append(
            models.Driver.name.ilike(pattern)
            | models.Driver.license_number.ilike(pattern)
            | models.Driver.phone.ilike(pattern)
        )

    count_stmt = select(func.count()).select_from(models.Driver)
    list_stmt = select(models.Driver)
    for f in filters:
    ...
    result = await db.execute(
        list_stmt.order_by(models.Driver.id.asc()).limit(pagination.limit).offset(pagination.offset)
    )
    return schemas.Page(
