...
async def update_shipment(
    ...
    """
    Partially update a shipment. Only fields present in the request body
    are changed (`exclude_unset=True`) — omitted fields are left as-is,
    and explicit `null` is honored where the field is nullable.

    Validation before any write:
    - driver_id / vehicle_id, if changed, must reference existing rows (404)
    - status changes must follow the allowed lifecycle
      (pending -> in_transit -> delivered, or -> cancelled from pending/in_transit);
      delivered/cancelled are terminal (409 on any attempted change)
    - field-level constraints (priority 1-3, etc.) enforced by ShipmentUpdate (422)
    """
    result = await db.execute(
        select(models.Shipment)
        .options(selectinload(models.Shipment.route))
    ...
    if "driver_id" in updates and updates["driver_id"] is not None:
        await _get_driver_or_404(db, updates["driver_id"])
    if "vehicle_id" in updates and updates["vehicle_id"] is not None:
        await _get_vehicle_or_404(db, updates["vehicle_id"])

    if "status" in updates:
        new_status = models.ShipmentStatus(updates["status"])
        updates["status"] = new_status  # ensure the ORM enum type is what gets assigned
        ...
        # Any other field edit (driver, vehicle, priority, timestamps...) is also
        # blocked once a shipment is in a terminal state — it's considered closed.
        raise ConflictError(
...
async def delete_shipment(shipment_id: int, db: AsyncSession = Depends(get_db)) -> None:
    """
    Delete a shipment.

    Explicit rule: only 'pending' shipments can be deleted. Once a shipment
    is in_transit or further, it's real-world activity that happened — it
    must be cancelled via PATCH (status=cancelled), not erased. Deleting a
    pending shipment cascades to its Route and any Alerts (configured via
    `cascade="all, delete-orphan"` in the ORM relationship), so no orphaned
    rows are left behind.
    """
    result = await db.execute(select(models.Shipment).where(models.Shipment.id == shipment_id))
    shipment = result.scalar_one_or_none()
    if shipment is None:
        raise NotFoundError(f"Shipment {shipment_id} not found.", error_code="shipment_not_found")
